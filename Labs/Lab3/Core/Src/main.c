...
int main(void)
{
  ...
  MX_TIM4_Init();
  /* USER CODE BEGIN 2 */
  HAL_TIM_Base_Start_IT(&htim4);					//Start the timer interrupt
  HAL_UART_Receive_IT(&huart2, RXBuffer, 1);		//Start receiving in interrupt mode
  CLI_Init();
  CLI_ExecuteCommand(4);   // Print Help Message
  /* USER CODE END 2 */
  ...
  while (1)
  {
	  uint32_t timerValue = __HAL_TIM_GET_COUNTER(&htim4);
	  if (timerValue >= 999){
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart) {
        txComplete = 1;  // Set the flag when transmission completes
}
